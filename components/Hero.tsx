...
const Hero: React.FC = () => {
    ...
    return (
        <section id="hero" className="min-h-[calc(100vh-80px)] flex flex-col justify-center items-center text-center py-20">
            ...
            <h1 className="max-w-4xl mx-auto text-3xl sm:text-4xl md:text-5xl font-extrabold text-on-surface-light dark:text-on-surface-dark mb-4 animate-fade-in-up [animation-delay:0.2s]">Chien Escalera Duong Front-End Developer • AI Builder (ex-Hollywood Stunt Performer)</h1>
            <p className="max-w-3xl mx-auto text-lg sm:text-xl md:text-2xl text-on-surface-variant-light dark:text-on-surface-variant-dark mb-8 animate-fade-in-up [animation-delay:0.4s]">I turn product ideas into elegant, fast interfaces—delivered with AI-accelerated workflows. 10 yrs on film sets (Avatar 2), now shipping for startups & teams.</p>
